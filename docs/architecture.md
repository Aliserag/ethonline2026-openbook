...
flowchart LR
    ...
    subgraph Arc["Arc Testnet (5042002)"]
        ...
        HOOK["SlaHook.sol , urs (EIP-8183 IACPHook)<br/>beforeAction(complete) verifies the attestation<br/>covers the payloadHash ∧ metaBlock ≥ minBlock<br/>else revert SlaNotMet(metaBlock, minBlock)"]
...
1. **Resolve** , he buyer reads `openbook.eth`'s `svc.*` text records on Sepolia
...
2. **Quote** , get_quote` parses `svc.price` ("0.10 USDC/query") into 6-decimal
   ...
   freshness floor and escrow deadline. Data-only , o writes.
3. **Pay** , he buyer funds an ERC-8183 job whose description commits the SLA
   (`{"minBlock":N,"schemaHash":"0x…","maxLatencyMs":M}`). Split-key: the buyer
   signs `createJob`/`approve`/`fund`; the seller signs `setBudget`.
4. **Deliver** , he seller runs the dataset query through the Gateway, the
...
5. **Verify** , he buyer re-runs the deterministic verdict
   (`metaBlock ≥ minBlock` ∧ well-formed hash): `APPROVE` → `complete()`
   (PaymentReleased to the seller), `REJECT` → `rejectAndRefund()`
   (Refunded , he money shot). Timed out jobs resolve via `claimRefund()`.
6. **P&L** , he arc-testnet subgraph indexes only OpenBook's own contract
   events (never raw USDC `Transfer`s , he EIP-7708 double-count trap) into
...
`REJECT (STALE_DATA)`, and the escrow fires the onchain refund , he same
...
- SLA conditions (freshness block, deliverable hash, deadline) are committed at
  ...
  committed at `submit`.
- **The SLA is enforced onchain, not by the client.** `SlaHook.sol` (ours , n
  ...
  the buyer's CLI is compromised or replaced , he refund path stays open.
...
- USDC is always displayed in the 6-decimal ERC-20 view . rc's native-gas
